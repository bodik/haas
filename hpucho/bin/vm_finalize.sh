...
if [ -f ${INSTALL_DIR}/bin/uchotcp.py ]; then
	if [ -z "${AUTOTEST}" ]; then
		/puppet/jenkins/bin/haas_vm_finalize_racert.sh -i $INSTALL_DIR -w $WARDEN_SERVER_URL -n "${CLIENT_NAME}.uchotcp"
	fi
...
if [ -f ${INSTALL_DIR}/bin/uchoudp.py ]; then
	if [ -z "${AUTOTEST}" ]; then
		/puppet/jenkins/bin/haas_vm_finalize_racert.sh -i $INSTALL_DIR -w $WARDEN_SERVER_URL -n "${CLIENT_NAME}.uchoudp"
	fi
