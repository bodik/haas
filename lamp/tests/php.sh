...
#php engine, tuning
TMPFILE=$(mktemp)
curl --silent --insecure --include "https://$(facter fqdn)/haas/test/binfo.php" > ${TMPFILE}
if [ $? -ne 0 ]; then
...
#app engine tests
curl --silent --insecure --include "https://$(facter fqdn)/haas/test/test_mysql_functions.php" | grep 'mysql functions present'
if [ $? -ne 0 ]; then
