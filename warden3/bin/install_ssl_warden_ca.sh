...
#TODO: (in)secure
curl --insecure --fail --data-urlencode @"${FQDN}.csr" "${WARDEN_CA_URL}/put_csr?client_name=$CLIENT_NAME"
if [ $? -ne 0 ]; then
...
while [ ${SIGNED} -eq 0 ]; do
	curl --insecure --output "${FQDN}.crt" "${WARDEN_CA_URL}/get_crt?client_name=$CLIENT_NAME" 2>/dev/null
