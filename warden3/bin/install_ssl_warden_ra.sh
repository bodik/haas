...
curl "$WARDEN_CA_URL/getToken?name=${CLIENT_NAME}&password=${TOKEN}" 1>/dev/null 2>/dev/null
if [ $? -ne 0 ]; then
	echo "ERROR: cannot contact warden_ca"
	exit 1
fi

bash /puppet/warden3/files/opt/warden_ra/warden_apply.sh ${WARDEN_CA_URL}/getCert ${CLIENT_NAME} ${TOKEN}
if [ $? -ne 0 ]; then
	echo "ERROR: cannot contact warden_ca"
	exit 1
fi

curl --insecure --output "cachain.pem" "${WARDEN_CA_URL}/getCacert?password=${TOKEN}" 2>/dev/null
