...
usage() { echo "Usage: $0 -w WARDEN_SERVER -n CLIENT_NAME -s SECRET -d DESTDIR" 1>&2; exit 1; }
while getopts "w:n:s:d:" o; do
	case "${o}" in
...
test -n "$SECRET" || rreturn 1 "ERROR: missing SECRET"
...
URL="${CA_SERVICE}/register_sensor?client_name=${CLIENT_NAME}&secret=${SECRET}"
